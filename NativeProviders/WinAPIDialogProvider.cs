...
{
    public bool CurrentPlatformSupported => OperatingSystem.IsWindowsVersionAtLeast(6, 0, 6000);
    ...
    private static void AddFiltersToDialog(IFileDialog dialog, NativeFileDialog.Filter[] filters)
    {
        ...
        for (int i = 0; i < filters.Length; i++)
        {
            ...
            unsafe
            {
                fixed (char* pFilter = filter)
                    specList[i].pszSpec = new(pFilter);
                fixed (char* pName = filters[i].Name)
                    specList[i].pszName = new(pName);
            }
        }
        ...
        unsafe
        {
            fixed (char* pWildcard = WILDCARD)
                specList[filters.Length].pszSpec = new(pWildcard);
            fixed (char* pWildcardName = WILDCARD_NAME)
                specList[filters.Length].pszName = new(pWildcardName);
        }
